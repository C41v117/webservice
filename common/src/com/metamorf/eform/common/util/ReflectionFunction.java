...
public class ReflectionFunction {
    ...
    public static Object getPropertyValue(Class domainClass, String prop){
    	...
    	try {
			...
			e.printStackTrace();
			try {
				LOGGER.error("Failed setting domain property due to field {} is inaccessible.", URLEncoder.encode(StringUtils.trimToEmpty(prop), "UTF-8"));
			} catch (UnsupportedEncodingException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
    	...
    	try {
			...
			if (field.getType().isAssignableFrom(Integer.class) ||
				...
				field.setLong(null, Integer.valueOf(value));
					...
					field.getType().isAssignableFrom(boolean.class)) {
				// sometimes we set 1 as boolean value in system parameter
				boolean alreadySet = false;
				try {
					int testValue = Integer.parseInt(value);
					// equals or lesser than 0 is treated as false
					if (testValue > 0) {
			...
			try {
				LOGGER.error("Field {} has not been declared yet in {}.", new Object[] {URLEncoder.encode(prop, "UTF-8"), domainClass.getName()});
			} catch (UnsupportedEncodingException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			...
			e.printStackTrace();
			try {
				LOGGER.error("Illegal value is given for field {}. The value given is {}", new Object[] {URLEncoder.encode(StringUtils.trimToEmpty(prop), "UTF-8"), URLEncoder.encode(StringUtils.trimToEmpty(value), "UTF-8")});
			} catch (UnsupportedEncodingException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
